...
### VEG COMMUNITIES ONLY

### Plug in your community dataframe (could try matrix, I found it worked with a dataframe)
comm <- data.frame(species_s)

### Plug in your trait dataframe
df <- traits_s

### Set the number of replicates for signficance testing (nothing above 1000 for our purposes)
nreps <- 999

### Run for each different trait, saving the results in a trait-specific dataframe


trait <- "seed_mass"
results_seedmass <- null.mom(comm,trait,df,nreps)

trait <- "RMR"
results_RMR <- null.mom(comm,trait,df,nreps)

trait <- "RDMC"
results_RDMC <- null.mom(comm,trait,df,nreps)

trait <- "SRL"
results_SRL <- null.mom(comm,trait,df,nreps)

trait <- "SLA"
results_SLA <- null.mom(comm,trait,df,nreps)

trait <- "LDMC"
results_LDMC <- null.mom(comm,trait,df,nreps)

trait <- "Rdiam"
results_Rdiam <- null.mom(comm,trait,df,nreps)


### Combine all CWV columns into a single dataframe
CWM_V <- cbind(results_seedmass$obs.cwv, results_RMR$obs.cwv, results_RDMC$obs.cwv, results_SRL$obs.cwv, results_SLA$obs.cwv, results_LDMC$obs.cwv, results_Rdiam$obs.cwv)
CWM_V <- data.frame(CWM_V)

### Optional - set column names and rownames
colnames(CWM_V) <- c("seedmass_CWV", "RMR_CWV", "RDMC_CWV", "SRL_CWV", "SLA_CWV", "LDMC_CWV","Rdiam_CWV")
rownames(CWM_V) <- c()




### FUNCTION (DO NOT CHANGE OR DELETE)
